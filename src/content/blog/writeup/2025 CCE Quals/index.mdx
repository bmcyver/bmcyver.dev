...
1) **DNS exfiltration** (DNS는 UDP 기반)  
...
네트워크 아웃바운드가 UDP만 가능하니 **DNS exfiltration XSS**를 해야한다.
...
`user.CouponUsed`는 루프 안에서 매번 `true`로 고정하지 않고, 루프 종료 후 한 번만 `true`로 설정한다.  
